...
document.addEventListener("DOMContentLoaded", () => {
    firebase.auth().onAuthStateChanged((user) => {
        if (!user) {
            // ❌ المستخدم غير مسجل -> حجب الموقع وإظهار زر "سجل الآن"
            document.body.innerHTML = ''; // إزالة المحتوى لحجب الموقع
            ...
            message.innerHTML = `<p>يجب عليك تسجيل الدخول للوصول إلى الموقع</p>`;
            ...
            // عند النقر، التوجيه إلى صفحة التسجيل
            registerBtn.addEventListener("click", () => {
                window.location.href = "Pages/login.html";
            });

            // إضافة العناصر إلى الصفحة
            message.appendChild(registerBtn);
    ...
    if (logoutBtn) {
        logoutBtn.addEventListener("click", () => {
            firebase.auth().signOut().then(() => {
                window.location.href = "Pages/login.html"; // ⬅️ التوجيه بعد تسجيل الخروج
            }).catch((error) => {
